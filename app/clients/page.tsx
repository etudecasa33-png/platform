              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="absolute top-5 right-5 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition z-10"
                  >
                    <Pencil size={16}/>
                  </button>
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleDeleteClient(client.id, client.name); }} 
                    className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition z-10"
                  >
                    <Trash2 size={16}/>
                  </button>
                </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl relative overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={editingClientId ? handleUpdateClient : handleCreateClient} className="p-6 space-y-4 overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2"><label className="block text-sm font-bold text-gray-700 mb-1">Remarques / Notes</label><textarea value={formState.remarques} onChange={e => setFormState({...formState, remarques: e.target.value})} className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none h-24" /></div>
              </div>
